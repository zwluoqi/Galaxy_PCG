...
Varyings CustomLitPassVertex(Attributes input)
{
    ...
    float ocean = invLerp(_minmax.x,0,output.uv.y);
    float top = invLerp(0,_minmax.y,output.uv.y);
...
half4 CustomLitPassFragment(Varyings input) : SV_Target
{
    ...
    surfaceData.smoothness *= input.color.x;
    ...
    //return float4(input.color.x,0,0,0);
    return color;
}
